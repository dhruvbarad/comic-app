  ...
  if (type === "marvel") {
    try {
      const response = await fetch(
        `${MARVEL_API_BASE_URL}/v1/public/characters/${id}/comics?limit=5&apikey=${PUBLIC_KEY}&hash=${API_HASH}&ts=1&orderBy=-focDate`
      );
      const result: any = await response.json();
      if (result.data.results.length === 0) {
        return { code: 404 };
      }
      const comics = result.data.results.map(
        (comic: any) =>
          new Object({
  ...
  if (type === "dc" || type === "star-wars") {
    try {
      ...
      const intelligence = new Object({
        copyRightHTML:
          '<a href="https://superheroapi.com">Data provided by superheroapi.com</a>',
        title: "Intelligence",
